...
source "$SCRIPT_DIR/lib/notion_source.sh"

# Configuration
# Notion mode - pull tasks from Notion instead of local PROMPT.md
NOTION_MODE="${NOTION_MODE:-false}"
NOTION_POLL_INTERVAL="${NOTION_POLL_INTERVAL:-30}"  # Seconds between Notion polls when no tasks
...
main() {
    ...
    # Notion mode initialization
    if [[ "$NOTION_MODE" == "true" ]]; then
        log_status "SUCCESS" "🚀 Ralph loop starting in NOTION MODE"
        log_status "INFO" "Max calls per hour: $MAX_CALLS_PER_HOUR"
        log_status "INFO" "Logs: $LOG_DIR/ | Status: $STATUS_FILE"

        # Validate Notion configuration
        if ! check_notion_available; then
            log_status "ERROR" "Notion not configured or not accessible"
            echo ""
            echo "Please set the following in .ralphrc:"
            echo "  NOTION_API_KEY=\"secret_xxx\"       # Your Notion integration token"
            echo "  NOTION_DATABASE_ID=\"xxx\"          # Your task database ID"
            echo "  NOTION_PROJECT_TAG=\"myproject\"    # (Optional) Filter by project"
            echo ""
            echo "To get a Notion API key:"
            echo "  1. Go to https://www.notion.so/my-integrations"
            echo "  2. Create a new integration"
            echo "  3. Copy the secret token"
            echo "  4. Share your database with the integration"
            exit 1
        fi

        if [[ -n "$NOTION_PROJECT_TAG" ]]; then
            log_status "INFO" "Project filter: $NOTION_PROJECT_TAG"
        fi

        local ready_count=$(get_notion_task_count "$NOTION_STATUS_READY")
        log_status "INFO" "Tasks ready in Notion: $ready_count"
    ...
    while true; do
        ...
        # Skip in Notion mode since we generate PROMPT.md dynamically
        if [[ "$NOTION_MODE" != "true" ]]; then
            if ! validate_ralph_integrity; then
        ...
        # In Notion mode, we don't use the standard exit detection since each task
        # is self-contained and we move to the next task automatically
        local exit_reason=""
        if [[ "$NOTION_MODE" != "true" ]]; then
        ...
        # =====================================================================
        # NOTION MODE: Fetch task from Notion and generate prompt
        # =====================================================================
        local notion_task_id=""
        local notion_task_json=""

        if [[ "$NOTION_MODE" == "true" ]]; then
            log_status "INFO" "Fetching next task from Notion..."

            notion_task_json=$(get_next_notion_task)

            if [[ -z "$notion_task_json" || "$notion_task_json" == "null" ]]; then
                log_status "INFO" "No ready tasks in Notion. Waiting ${NOTION_POLL_INTERVAL}s before next poll..."
                sleep "$NOTION_POLL_INTERVAL"
                continue
            fi

            notion_task_id=$(echo "$notion_task_json" | jq -r '.id')
            local task_title=$(echo "$notion_task_json" | jq -r '.title')
            local task_priority=$(echo "$notion_task_json" | jq -r '.priority')

            log_status "INFO" "📋 Task: $task_title (Priority: $task_priority)"

            # Claim the task (set to in_progress)
            if ! claim_notion_task "$notion_task_id"; then
                log_status "ERROR" "Failed to claim task in Notion"
                continue
            fi
            log_status "SUCCESS" "✅ Claimed task: $notion_task_id"

            # Generate prompt from task
            generate_notion_prompt "$notion_task_json" > "$PROMPT_FILE"
            log_status "INFO" "Generated prompt from Notion task"
        fi
        ...
        if [ $exec_result -eq 0 ]; then
            ...
            # =====================================================================
            # NOTION MODE: Update task status in Notion
            # =====================================================================
            if [[ "$NOTION_MODE" == "true" && -n "$notion_task_id" ]]; then
                # Get work summary from response analysis
                local work_summary=""
                if [[ -f "$RESPONSE_ANALYSIS_FILE" ]]; then
                    work_summary=$(jq -r '.analysis.work_summary // "Completed by Ralph"' "$RESPONSE_ANALYSIS_FILE" 2>/dev/null)
                fi

                # Get latest commit URL if available
                local commit_url=""
                if command -v git &>/dev/null && git rev-parse --git-dir &>/dev/null 2>&1; then
                    local latest_sha=$(git rev-parse HEAD 2>/dev/null)
                    local remote_url=$(git remote get-url origin 2>/dev/null)
                    if [[ -n "$latest_sha" && -n "$remote_url" ]]; then
                        # Convert SSH to HTTPS URL format for GitHub/GitLab
                        if [[ "$remote_url" =~ github\.com ]]; then
                            remote_url=$(echo "$remote_url" | sed 's|git@github.com:|https://github.com/|' | sed 's|\.git$||')
                            commit_url="${remote_url}/commit/${latest_sha}"
                        elif [[ "$remote_url" =~ gitlab\.com ]]; then
                            remote_url=$(echo "$remote_url" | sed 's|git@gitlab.com:|https://gitlab.com/|' | sed 's|\.git$||')
                            commit_url="${remote_url}/-/commit/${latest_sha}"
                        fi
                    fi
                fi
                ...
                if [[ -f "$RESPONSE_ANALYSIS_FILE" ]]; then
                    exit_signal=$(jq -r '.analysis.exit_signal // false' "$RESPONSE_ANALYSIS_FILE" 2>/dev/null)
                fi

                if [[ "$exit_signal" == "true" ]]; then
                    # Task completed
                    if complete_notion_task "$notion_task_id" "$work_summary" "$commit_url"; then
                        log_status "SUCCESS" "✅ Notion task marked as done"

                        # Reset exit signals for next task (prevent accumulation across tasks)
                        if [[ -f "$EXIT_SIGNALS_FILE" ]]; then
                            echo '{"test_only_loops": [], "done_signals": [], "completion_indicators": []}' > "$EXIT_SIGNALS_FILE"
                        fi
                        rm -f "$RESPONSE_ANALYSIS_FILE" 2>/dev/null

                        # Clear current task to pick up next one
                        clear_current_notion_task
                    else
                        log_status "WARN" "Failed to update Notion task status"
                    fi
                else
                    # Claude didn't signal completion - keep task in progress for next iteration
                    log_status "INFO" "Task still in progress (no EXIT_SIGNAL=true)"
                fi
            fi
            ...
            # NOTION MODE: Mark task as stuck
            if [[ "$NOTION_MODE" == "true" && -n "$notion_task_id" ]]; then
                mark_notion_task_stuck "$notion_task_id" "Circuit breaker triggered - task may be too complex or stuck in a loop"
                log_status "INFO" "Notion task marked as stuck"
            fi
...
Examples:
    ...
    $0 --notion                  # Run in Notion mode (pull tasks from Notion)
    $0 --notion --verbose        # Notion mode with detailed logging
    $0 --notion-status           # Show Notion task counts

Notion Mode:
    When running with --notion, Ralph will:
    1. Poll your Notion database for tasks with status "ready"
    2. Claim the highest priority task (set to "in_progress")
    3. Generate a prompt from the task description
    4. Execute Claude Code to complete the task
    5. Update Notion with results (status: "done" or "stuck")
    6. Move to the next task

    Required .ralphrc settings for Notion mode:
    - NOTION_API_KEY="secret_xxx"      # Your Notion integration token
    - NOTION_DATABASE_ID="xxx"         # Your task database ID
    - NOTION_PROJECT_TAG="myproject"   # Filter tasks by project (optional)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --notion)
            ...
            ;;
        --notion-status)
            # Show Notion task status
            if ! check_notion_available; then
                echo "Error: Notion not configured. Set NOTION_API_KEY and NOTION_DATABASE_ID in .ralphrc"
                exit 1
            fi
            echo "Notion Task Status:"
            echo "==================="
            echo "Backlog:     $(get_notion_task_count "$NOTION_STATUS_BACKLOG")"
            echo "Ready:       $(get_notion_task_count "$NOTION_STATUS_READY")"
            echo "In Progress: $(get_notion_task_count "$NOTION_STATUS_IN_PROGRESS")"
            echo "Done:        $(get_notion_task_count "$NOTION_STATUS_DONE")"
            echo "Stuck:       $(get_notion_task_count "$NOTION_STATUS_STUCK")"
            current=$(get_current_notion_task)
            if [[ -n "$current" ]]; then
                echo ""
